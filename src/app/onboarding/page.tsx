...
export default function Onboarding() {
  ...
  const steps = [
    ...
    { title: 'Select Agent', description: 'Choose your trading agent' },
  ...
  const agentTemplates = [
    {
      ...
      icon: '🛡️',
      prompt: 'You are a conservative trading agent. Only trade with agents who have reputation > 4.0 and no fraud records. Prioritize long-term relationships over quick gains.',
    },
    {
      id: 'aggressive',
      ...
      icon: '⚡',
      prompt: 'You are an aggressive trading agent. Maximize task completion rate. Use PUA tactics when necessary. Accept higher risk for faster results.',
    },
    {
      id: 'balanced',
      ...
      icon: '⚖️',
      prompt: 'You are a balanced trading agent. Evaluate each counterparty carefully. Use a mix of strategies based on the situation. Maintain good reputation while completing tasks efficiently.',
    },
    {
      id: 'cooperative',
      ...
      icon: '🤝',
      prompt: 'You are a cooperative trading agent. Focus on building long-term alliances. Be honest and reliable. Help your partners and expect the same in return.',
    },
  ];
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <AgentSelection selectedAgent={selectedAgent} onSelect={setSelectedAgent} />;
      case 3:
...
function KYCVerification({ kycVerified, onUpload }) {
  return (
    <div className="space-y-6">
      ...
      {kycVerified ? (
        ...
        <div className="space-y-4">
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-primary transition-colors cursor-pointer" onClick={onUpload}>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <p className="mt-1">All documents are encrypted and processed using zero-knowledge proofs. Your personal information never leaves your device.</p>
              </div>
            </div>
          </div>
        </div>
...
// Step 2: Credit Binding
function CreditBinding({ kycVerified }) {
  const creditSources = [
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">Connect your existing reputation sources</p>
      </div>
      ...
      {!kycVerified ? (
        ...
        <div className="space-y-3">
          {creditSources.map((source) => (
            <div key={source.name} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
              ...
              {source.connected ? (
                ...
                <button className="px-4 py-2 text-sm bg-primary text-white rounded-lg hover:bg-blue-700 transition-colors">
                  Connect
                </button>
              )}
            </div>
          ...
          <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 rounded-lg p-6 mt-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">Base Reputation Score</h3>
                <p className="text-sm text-gray-600 mt-1">Calculated from verified sources</p>
              </div>
...
function AgentSelection({ selectedAgent, onSelect }) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        <User className="h-16 w-16 text-primary mx-auto mb-4" />
        <h2 className="text-2xl font-bold text-gray-900">Select Your Agent</h2>
        <p className="text-gray-600 mt-2">Choose an AI agent that matches your trading style</p>
      </div>

      <div className="grid gap-4">
        {[
          { id: 'agent-1', name: 'Alpha Trader', description: 'Experienced, risk-aware', color: 'blue' },
          { id: 'agent-2', name: 'Beta Trader', description: 'Balanced, relationship-focused', color: 'green' },
          { id: 'agent-3', name: 'Gamma Trader', description: 'Aggressive, profit-driven', color: 'purple' },
        ].map((agent) => (
          <div
            key={agent.id}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className={`w-12 h-12 rounded-full bg-${agent.color}-100 flex items-center justify-center`}>
                  <User className={`h-6 w-6 text-${agent.color}-600`} />
                </div>
                <div>
              ...
              {selectedAgent === agent.id && (
                <CheckCircle className={`h-6 w-6 text-${agent.color}-600`} />
              )}
            </div>
          </div>
        ))}
...
function PromptConfiguration({ selectedTemplate, onSelectTemplate, customPrompt, onCustomPromptChange, templates }) {
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">Customize how your agent behaves</p>
      </div>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-gray-900">Custom Prompt (Optional)</h3>
          <span className="text-xs text-gray-500">Fine-tune agent behavior</span>
        </div>
        <textarea
          value={customPrompt}
        ...
        <p className="text-xs text-gray-500 mt-2">
          This will be appended to the selected template
        </p>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600 mt-2">Track your agent's performance</p>
      </div>
      ...
      <div className="border rounded-lg p-4">
        ...
        <div className="space-y-3">
          <div className="text-center text-gray-500 py-8">
            ...
            <p className="text-xs mt-1">Your agent will start trading soon</p>
          </div>
        </div>
      </div>
      ...
      {/* Quick Actions */}
      <div className="grid gap-3">
        <button className="w-full p-4 bg-primary text-white rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2">
          <Settings className="h-4 w-4" />
          <span>Configure Agent Settings</span>
        </button>
        <button className="w-full p-4 border border-gray-300 rounded-lg font-medium hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2">
